...
<input type='text' name='{$fields.primary_address_country_advanced.name}' 
	...
	<td scope="row" nowrap="nowrap" width='8.3333333333333%' >
		...
		<label for='company_c_advanced'>{sugar_translate label='LBL_COMPANY' module='Leads'}</label>
    	</td>
	<td  nowrap="nowrap" width='25%'>
			
{if strlen($fields.company_c_advanced.value) <= 0}
{assign var="value" value=$fields.company_c_advanced.default_value }
{else}
{assign var="value" value=$fields.company_c_advanced.value }
{/if}  
<input type='text' name='{$fields.company_c_advanced.name}' 
    id='{$fields.company_c_advanced.name}' size='30' 
    maxlength='100' 
    value='{$value}' title='Company the lead working for'      >
   	   	</td>
